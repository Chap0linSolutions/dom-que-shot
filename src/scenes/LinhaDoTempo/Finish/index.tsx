...
const getOrganizedResults = (results: Results) => {
  const validGuesses = results.guesses.filter(
    (g) => g.guess !== Status.TIMESUP && g.guess !== Status.DISCONNECTED
  );
...
const getGuidanceText = (
  ...
  if (whoPlayed.length === 0) return <>Tragédia total! Todo mundo bebe.</>;
  ...
  if (whoPlayed.length === 1) {
    return (
      <>
        Mas só um respondeu? Aff
        <br />
        Quem não jogou a tempo bebe.
      </>
    );
  }
  ...
  if (top.length === whoPlayed.length) {
    return bullseye ? (
      <>
        Todo mundo acertou em cheio!
        <br />
        Neste caso, ninguém bebe. Nerds.
      </>
    ) : (
      <>
        Todo mundo empatou!
        <br />
        Neste caso, ninguém bebe.
      </>
    );
  }
  ...
  if (top.length > 1) {
    return bullseye ? (
      <>
        {top.length} jogadores acertaram em cheio!
        <br />
        Neste caso, quem não empatou bebe 2x.
      </>
    ) : (
      <>
        Um empate! que coisa mais linda.
        <br />
        Neste caso, quem não empatou bebe.
      </>
    );
  }
  return bullseye ? (
    <>
      {top[0].player} acertou em cheio!
      <br />
      Neste caso, os demais bebem 2x.
    </>
  ) : (
    <>
      {top[0].player} foi quem chegou mais perto!
      <br />
      Neste caso, todos os demais bebem.
    </>
  );
};
...
export default function FinishPage({
  ...
  const dnf = useRef(
    results.guesses.filter((p) => p.guess === Status.TIMESUP)
  ).current;
  const disc = useRef(
    results.guesses.filter((p) => p.guess === Status.DISCONNECTED)
  ).current;
        ...
        <Content>
          <AnswerDiv ref={answerDivRef}>
            <AnswerTile ref={tileRef}>
              <AnswerText>e a resposta era:</AnswerText>
          ...
          <GuidanceText ref={textRef}>{guidanceText}</GuidanceText>
        </Content>
