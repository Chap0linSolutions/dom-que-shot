...
export default function FinishPage({
  ...
  useLayoutEffect(() => {   
    ...
    const roleAnimation = gsap.timeline()
    .to('.role', {
      opacity: 0,
      duration: 0,
    })
    ...
    const nickAnimation = gsap.timeline()
    .to('.nickname', {
      opacity: 0,
      duration: 0,
    })
  ...
  const winners = useRef<ListedPlayerProps[]>(players.filter(player => player.id === 1000));
  const losers = useRef<ListedPlayerProps[]>(players.filter(player => player.id === 0));

  const screenRatio = (window.innerHeight < 740)
  ? [0.35, 0.48]
  : [0.42, 0.5];

  const loserStyle = {
    maxHeight: (turnVisibility === true)
