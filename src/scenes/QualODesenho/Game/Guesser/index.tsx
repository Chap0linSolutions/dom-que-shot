    ...
    if (drawingPaths) {
      ...
      switch (type) {
        ...
        case 'my-canvas-width-is':
          interCanvasRatio.current = canvas.width / (payload as number);
          break;
        case 'paths':
          (payload as Path[]).forEach((path) => {
      ...
      <GameDiv>
        ...
        <GuessesAndCanvas>
          ...
          <GuessesDiv
            ...
            <GuessesTitle>Chutes</GuessesTitle>
            <Guesses style={{ maxHeight: 0.75 * canvas.height }}>
              {guesses.map((guess, i) => {
                if (playerGuessed && i === guesses.length - 1)
                  return <RightGuess key={i}>{guess}</RightGuess>;
        ...
        <GuessInputDiv>
          ...
          <Button
            ...
            width={`${innerWidth - (canvas.width + 28)}px`}
            height="50px"
            onClick={checkGuess}>
            Enviar
          </Button>
