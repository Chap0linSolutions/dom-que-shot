...
const description = (
  <>
    ...
    <br />
    <br />
    O jogador da vez irá receber algumas opções de palavra e deve escolher quais
    delas quer fazer mímica. Ele terá então 30 segundos para fazer seu show, e
    os demais jogadores vão tentar adivinhar.
    <br />
    <br />
    A mímica pode ser por gesto e/ou som (mas não vale falar). A roda precisa
    acertar pelo menos DUAS opções para que ninguém beba. Abaixo disso, todo
    mundo bebe uma dose para cada palavra não adivinhada.
    <br />
    <br />
    Boa sorte!
  </>
...
export default function MimicaMaster() {
  ...
  useEffect(() => {
    ...
    !user.isCurrentTurn &&
      socket.addEventListener('mimic-state-is', (ms) => {
        setMimicState(ms);
      });

    !user.isCurrentTurn &&
      socket.addEventListener('names-so-far', (n) => {
        setNamesSoFar(n);
      });
