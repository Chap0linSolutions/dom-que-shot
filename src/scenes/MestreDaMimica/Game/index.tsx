...
const SAND_LEVEL = 55;
const DELTA_SAND_LEVEL =
  ((IDLE_SAND_LEVEL / 2 + SAND_LEVEL) * DELTA_TIME) / GAME_DURATION;
...
export default function GamePage({
    ...
    if (turnVisibility) {
      ...
      setGuidanceText(
        <>
          ...
          <br />
          Não se esqueça de marcar os nomes
          <br />
          que forem sendo <Emphasis>adivinhados</Emphasis>.
        </>
      );
      return () => {
      ...
      setGuidanceText(
        <>
          <Emphasis>Vai começar em breve!</Emphasis>
          <br />
  ...
  useEffect(() => {
    ...
    gsap.to(progressRef.current, {
      width: correct.length > 1 ? '100%' : `${correct.length * 50 + 10}%`,
      duration: 1,
      ease: 'back',
    });
  ...
  const timeStamp =
    state === GameState.Idle ? (
      ...
      <RunningTimer>
        00:{msTimer >= 10000 ? msTimer / 1000 : `0${msTimer / 1000}`}
      </RunningTimer>
    );
  ...
  const ProgressText = () => { // TODO: return string
    switch (correct.length) {
      ...
      case 1:
        return <>Ainda vão beber 1 dose</>;
      case 2:
        return <>Yey! Todos venceram, 0 doses</>;
      default:
        return <>Acertaram todas! Vem monstro!</>;
    }
  };
  ...
  if (turnVisibility)
    return (
      <Background noImage>
        ...
        <Mimic>
          <Content>
            <Title>Suas opções são as seguintes:</Title>
            <SuggestionsDiv>
              {suggestions.map((s, i) => (
                <Item key={i} onClick={() => pushCorrect(i)}>
                  <ItemContent>
                    <Icon src={glassIcon} />
                    <CategoryAndWord>
                      <Word>{s}</Word>
    ...
    <Background noImage>
      ...
      <Mimic>
        ...
        <Content ref={guessProgressRef}>
          <GuessProgressTitle>PROGRESSO DO TIME:</GuessProgressTitle>
          <GuessProgressDiv>
            <ProgressTitle>
              <ProgressText />
            </ProgressTitle>
