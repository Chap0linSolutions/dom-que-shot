...
import { MainLogo, WelcomePage, Content, BurguerDiv, Options, Option } from './Welcome.style';
...
export default function Welcome({navigate, setRoom, setPage}: WelcomeProps){
  ...
  const toggleOptions = () => {
    ...
    setBurguerColor(previous => ((previous === colors.unselected)? colors.selected : colors.unselected ));
  }
  ...
  return (
      <Background>       
        <WelcomePage>
          <BurguerDiv>
            <Menu width={30} height={30} onClick={toggleOptions} color={burguerColor}/>
            {options && 
              <Options>
                <Option onClick={() => setPage(Pages.AboutUs)}>Sobre nós</Option>
                <Option onClick={() => setPage(Pages.Contact)}>Contato</Option>
                <Option onClick={() => setPage(Pages.PrivacyPolicy)}>Política de Privacidade</Option>
            </Options>}  
          </BurguerDiv> 
          <Content>
            <MainLogo src={logo} />
